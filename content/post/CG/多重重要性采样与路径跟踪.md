...
{{% notice info %}}
Todo: 一种特殊形式的MIS(权重分布为0-1分布)
{{% /notice %}}

<!-- 继续沿着$$\eqref{4}$$的思路。NEE的思路是每次光线按照BRDF采样的方向弹射，在每个击中的diffuse表面(`specular`表面会很特殊，其BRDF是个狄拉克函数所以需要额外的处理)都向光源方向采样一次。
把`indirect`和`direct`方向的`radiance`加起来。
注意如果通过`BRDF`采样得到的方向能直接命中光源的时候，此时不能进行光源采样，否则会出现`double-counting`的现象。

其以上描述的思路刚好是$$\eqref{4}$$的形式。
在$$\eqref{4}$$的代码实现中，每次弹射的方向由`BRDF`采样出来的方向决定。
其方向位于 $$\Omega^-$$ 区域时，也就是左半部分只应用`BRDF采样`，而对$$M$$区域只应用光源采样。
此时其采样策略(`indirect + direct`)是无偏的(积分域加起来为$$\Omega$$)。
对于BRDF采样能直接命中光源的时候，不能计算对光源采样的部分(因为此时采样方向不属于$$\Omega^-$$，不再满足$$\eqref{4}$$的形式)，此时其采样策略退化为单纯的`BRDF`采样。 -->
