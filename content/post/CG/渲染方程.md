...
$$
E(p) = \int_{\Omega^+} L_i(p,\omega)\cos\theta \text{d}\omega
$$

我们关心的是相机方向收到的*radiance*，而不是点p收到的*irradiance*，所以在上式中加入BRDF项，得到反射方程。

$$
L_r(p,\omega_r)= \int_{\Omega^+} f_r(\omega_i \Rarr \omega_r ) L_i(p,\omega)\cos\theta \text{d}\omega
$$
...
$$
L_r(p,\omega_r)= L_e(p,\omega_o) + \int_{\Omega^+} f_r(\omega_i \Rarr \omega_r ) L_i(p,\omega)\cos\theta \text{d}\omega
$$
